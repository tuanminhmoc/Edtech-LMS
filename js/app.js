...
function renderHistoryTablePaged() {
    ...
    if (!pageItems.length) tbody.innerHTML = '<tr><td colspan="7" class="history-empty-cell">Không có hoạt động phù hợp.</td></tr>';
        ...
        return `<tr class="history-session-card"><td class="history-col-time">${formatDate(item.timestamp)}</td><td class="history-col-mode"><span class="history-mode-badge ${isQuiz ? 'mode-quiz' : 'mode-flashcard'}">${isQuiz ? 'Trắc nghiệm' : 'Flashcard'}</span></td><td class="history-col-file"><strong title="${escapeHTML(item.file || 'Không tên')}">${escapeHTML(item.file || 'Không tên')}</strong></td><td class="history-col-result"><strong>${escapeHTML(result)}</strong></td><td class="history-col-duration">${formatLongDuration(item.duration || 0)}</td><td class="history-col-xp"><strong>+${Number(item.xp) || 0} XP</strong></td><td class="history-col-actions"><div class="history-actions"><button ${hasSource ? '' : 'disabled'} onclick="repeatHistorySession('${item.id}')">${isQuiz ? 'Làm lại' : 'Ôn lại'}</button>${isQuiz && Array.isArray(item.review) ? `<button onclick="reviewHistoryQuiz('${item.id}')">Xem lại</button>` : ''}<button ${hasFocusRows ? '' : 'disabled'} onclick="repeatHistorySession('${item.id}', true)">${isQuiz ? 'Câu sai' : 'Thẻ khó'}</button><button class="danger" onclick="deleteHistoryItem('${item.id}')">Xóa</button></div></td></tr>`;
    }).join('');
